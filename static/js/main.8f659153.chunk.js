...
class PageProvider extends react__WEBPACK_IMPORTED_MODULE_0__["Component"] {
  constructor(...args) {
    ...
    this.componentDidMount = () => {
      this.unsubscribeFromPages = this.getFrontPage().then(() => this.getNumPages('pages')).then(numPages => this.fetchPages(numPages, 'pages')).catch(err => console.error('err fetching page data', err));
      this.unsubscribeFromPosts = this.getFrontPage().then(() => this.getNumPages('pages')).then(numPosts => this.fetchData(numPosts, 'pages')).then(data => this.updateState(data, 'pages')).catch(err => console.error('err fetching post data', err));
    };
...
//# sourceMappingURL=main.8f659153.chunk.js.map