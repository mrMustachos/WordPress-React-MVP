...
class PageProvider extends react__WEBPACK_IMPORTED_MODULE_0__["Component"] {
  constructor(...args) {
    ...
    this.fetchData = async type => {
      ...
      const perPage = this.getPerPage(restType);
      const url = "".concat(baseUrl).concat(restType).concat(perPage);
      const responce = await axios__WEBPACK_IMPORTED_MODULE_1___default()(url, wpFetchHeaders);
      return this.setDataPaths(restType, responce);
    };
    ...
    this.collectAllData = async numPages => {
      ...
      for (let pg = 1; pg <= numPages; pg += 1) {
        const url = "".concat(baseUrl).concat(restType).concat(perPage, "&page=").concat(pg);
    ...
    this.componentDidMount = () => {
      ...
      .then(() => this.fetchData()).then(numPages => this.collectAllData(numPages)).then(data => this.updateState(data)).catch(err => console.error('err fetching page data', err));
    };
...
//# sourceMappingURL=main.f6912059.chunk.js.map