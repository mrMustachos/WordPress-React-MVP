...
class PageProvider extends react__WEBPACK_IMPORTED_MODULE_0__["Component"] {
  constructor(...args) {
    ...
    this.getNumPages = async () => {
      ...
      const url = "".concat(baseUrl, "pages").concat(perPage);
    ...
    this.fetchPages = async numPages => {
      ...
      for (let pg = 1; pg <= numPages; pg += 1) {
        const url = "".concat(baseUrl, "pages").concat(perPage, "&page=").concat(pg);
        const page = axios__WEBPACK_IMPORTED_MODULE_1___default.a.get(url, wpFetchHeaders);
        pagesArr.push(page);
      }

      await axios__WEBPACK_IMPORTED_MODULE_1___default.a.all(pagesArr).then(response => {
    ...
    this.componentDidMount = () => {
      this.unsubscribeFromPages = this.getFrontPage().then(() => this.getNumPages()).then(numPages => this.fetchPages(numPages)).catch(err => console.error('err fetching page data', err));
    };
...
//# sourceMappingURL=main.56a366bf.chunk.js.map