...
class PageProvider extends react__WEBPACK_IMPORTED_MODULE_0__["Component"] {
  constructor(...args) {
    ...
    this.dataInit = () => {
      this.getFrontPage().then(data => {
        console.log(this.props.match.path);
      }).catch(err => console.error('err fetching init data', err));
    };
    ...
    this.componentDidMount = () => {
      // this.unsubscribeFromPages = this.getFrontPage()
      // .then(() => this.getNumPages('pages'))
      // .then((numPages) => this.fetchAllData(numPages, 'pages'))
      // .then((data) => this.updateState(data, 'pages'))
      this.unsubscribeFromPages = this.dataInit().then(() => this.fetchData('pages')).then(numPages => this.collectAllData(numPages, 'pages')).then(data => this.updateState(data, 'pages')).catch(err => console.error('err fetching page data', err));
    };
...
//# sourceMappingURL=main.a9a1bf96.chunk.js.map