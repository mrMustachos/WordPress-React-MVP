...
const SearchForm = props => {
  const _useContext = Object(react__WEBPACK_IMPORTED_MODULE_0__["useContext"])(_providers_PageProvider_jsx__WEBPACK_IMPORTED_MODULE_2__["PageContext"]),
        term = _useContext.term,
        updateSearhTerm = _useContext.updateSearhTerm,
        submitSearch = _useContext.submitSearch;
...
/* harmony default export */ __webpack_exports__["default"] = (Object(react_router_dom__WEBPACK_IMPORTED_MODULE_1__["withRouter"])(SearchForm)); // export default WithConsumer(SearchForm);
...
class PageProvider extends react__WEBPACK_IMPORTED_MODULE_0__["Component"] {
  constructor(...args) {
    ...
    this.wpInfo = {
      ...
      post_perPage: '?per_page=6',
      page_perPage: '?per_page=10',
      wpFetchHeaders: {
        headers: {
    ...
    this.fetchData = async type => {
      ...
      return this.setDataPaths(type, responce);
    };

    this.collectAllData = async (numPages, type) => {
    ...
    this.componentDidMount = () => {
      this.unsubscribeFromPages = this.getFrontPage() // .then(() => this.getNumPages('pages'))
      // .then((numPages) => this.fetchAllData(numPages, 'pages'))
      // .then((data) => this.updateState(data, 'pages'))
      .then(() => this.fetchData('pages')).then(numPages => this.collectAllData(numPages, 'pages')).then(data => this.updateState(data, 'pages')).catch(err => console.error('err fetching page data', err));
    };
  ...
  // }
  getPerPage(type) {
    const _this$wpInfo4 = this.wpInfo,
          post_perPage = _this$wpInfo4.post_perPage,
          page_perPage = _this$wpInfo4.page_perPage;
    let contentType;

    switch (type) {
      case 'posts':
        contentType = post_perPage;
        break;

      case 'pages':
      default:
        contentType = page_perPage;
        break;
    }
    ...
    switch (type) {
      case 'posts':
        this.setState({
          ...
          post_totalPages: headers['x-wp-totalpages'],
          postArr: data
        });
        break;
      ...
      default:
        this.setState({
          ...
          page_totalPages: headers['x-wp-totalpages']
        });
        pathData = headers['x-wp-totalpages'];
        break;
    }
  ...
  getAllPosts() {
    this.unsubscribeFromPosts = this.fetchData('posts').then(result => result).catch(err => console.error('err fetching post data', err));
  }
...
const Archive = props => {
  ...
  return react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement("div", {
    ...
    __self: undefined
  }, "Search Results") : ''), react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement(_partials_Foot__WEBPACK_IMPORTED_MODULE_2__["default"], {
    __source: {
      fileName: _jsxFileName,
      lineNumber: 23
    },
...
//# sourceMappingURL=main.a795eb76.chunk.js.map