...
class PageProvider extends react__WEBPACK_IMPORTED_MODULE_0__["Component"] {
  constructor(...args) {
    ...
    this.getNumPages = async type => {
      ...
      const url = "".concat(baseUrl).concat(type).concat(perPage); // const { headers, data } = await Axios(url, wpFetchHeaders);
  ...
  getPerPage(type) {
    const _this$wpInfo5 = this.wpInfo,
          post_perPage = _this$wpInfo5.post_perPage,
          page_perPage = _this$wpInfo5.page_perPage;
    let contentType;
...
//# sourceMappingURL=main.0dfbba3a.chunk.js.map